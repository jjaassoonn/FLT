...
--     (S : Type u) [DivisionRing S] [Algebra K S]
--     (N : Type*) [Ring N] [Algebra K N] [Module S N] [IsSimpleModule S N] :
--     Nonempty (N ≃ₐ[K] S) := by
...
--     subst eq
--     let f : N ≃ₗ[S] S := b.repr ≪≫ₗ LinearEquiv.ofBijective
--       ({toFun := fun x => x ⟨i, by simp⟩
--         map_add' := fun x y => by simp
--         map_smul' := fun x y => by simp } : ((({i} : Set N) →₀ S) →ₗ[S] S))
--       ⟨fun x y hxy => by ext; simpa using hxy, fun x => ⟨Finsupp.single ⟨i, by simp⟩ x, by simp⟩⟩
--       -- ⟨⟨fun x => x ⟨i, by simp⟩, fun x y => by simp⟩, ?_⟩ ⟨?_, ?_⟩
...
example (k : K) (f : End $ ModuleCat.of R R) : true := by
  haveI : Algebra K (End $ ModuleCat.of R R) := inferInstance
  have := this.commutes
  simp only [End.mul_def] at this
...
instance : Ring (ModuleCat.of S Sᵐᵒᵖ) := inferInstanceAs $ Ring Sᵐᵒᵖ
instance : Algebra K (ModuleCat.of S Sᵐᵒᵖ) := inferInstanceAs $ Algebra K Sᵐᵒᵖ
#exit
...
noncomputable def toRingMopEquiv : Rᵐᵒᵖ ≃ₐ[K] Sᵐᵒᵖ :=
  mopEquivEnd K R |>.trans $
    aux1 K R S e |>.trans $
    aux2 K R S e |>.trans $
    -- AlgEquiv.trans _
    -- (mopEquivEnd K S).symm
